...
def generate_start_urls():
    ...
    end = int(df_start.iloc[4])
    no = 1000
    start = end - no
    print(start)
    return ['https://www.carnetauctions.com.au/cp_veh_inspection_report.aspx?sitekey=CSS&MTA={}'.format(cat_number) for cat_number in range(int(start),int(end))]
   
class CssSpider(scrapy.Spider):
    name = 'auto'
    callowed_domains = ['auto-auctions.com.au/']

...
def generate_start_urls():
    ...
    end = int(df_start.iloc[9])
    no = 1000
    start = end - no
    print(start)
    return ['https://www.f3motorauctions.com.au/cp_veh_inspection_report.aspx?sitekey=F3A&MTA={}'.format(cat_number) for cat_number in range(int(start),int(end))]

class F3aSpider(scrapy.Spider):
    name = 'auto'

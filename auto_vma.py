...
def generate_start_urls():
    ...
    end = int(df_start.iloc[5])
    no = 1000
    start = end - no
    print(start)
    return ['https://www.valleymotorauctions.com.au/cp_veh_Inspection_report.aspx?sitekey=VMA&MTA={}'.format(cat_number) for cat_number in range(int(start),int(end))]

class VmaSpider(scrapy.Spider):
    name = 'auto'

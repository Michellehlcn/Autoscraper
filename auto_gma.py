...
def generate_start_urls():
    ...
    end = int(df_start.iloc[7])
    no = 1000
    start = end - no
    print(start)
    return ['https://www.uaansw.com.au/cp_veh_inspection_report.aspx?sitekey=GMA&MTA={}'.format(cat_number) for cat_number in range(int(start),int(end))]
